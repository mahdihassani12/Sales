		    ...
		    <div class="col-md-6 feature">
			  <div class="row">
			     <div class="col-md-10" style="padding-left:2px; padding-right:43px;">
			      <img style="width:405px; max-height:400px;object-fit:cover; " src="@if($product->external_link==1) {{$product->image}} @else {{asset('public/images/product')}}/{{$product->image}} @endif" class="showBigPic">
			    </div>
			    <div class="col-md-2" style="padding-right:2px">
				  <div class="product_variation">
				  <img style=" border:1px solid black" src="@if($product->external_link==1) {{$product->image}} @else {{asset('public/images/product')}}/{{$product->image}} @endif">
			      <?php $counter=1; ?>
				  @foreach($productImages as $proImage)
                     @if($counter<4) 				  
				     <img src="@if($proImage->external_link!=1) {{asset('public/images/product_variation')}}/@endif {{$proImage->imag_gallery}}" >
   				     @else
					 @endif
                     <?php $counter++;?>				 
				  @endforeach
      ...
      <div class="modal-footer">
		...
		<input type="hidden" id="primary_selected_product_price" value="{{$product->price}}" category="{{$product->category_id}}">
		  ...
		  <?php 
			...
			//		 }
			   ?>
        <button  class="btn add_to_cartd" product_id="{{$product->id}}" product_qty="{{$product->storeqty}}">اطلب الان</button>
		...
		<div class="add_to_cart_alert alert alert-success alert-dismissable" style="display:none; background:transparent; border:none;">  
		 ...
		 <div class='social'>
		  ...
		  <ul class="link-list">
			...
			<li><a href="#" custome_link="{{asset('fronted/product')}}/{{$product->id}}"  class="copy_link_content" ><i class="fa fa-copy"></i> نسخ رابط المنتج</a></li>
		  </ul>
         </div>
     </div>
	@endforeach  
...
<div class="fronted-content">   
 <div class="fonted_products"> 
	  <h3 style="margin-bottom:10px;"> كل المنتجات  <span style="float:left; font-size:20px; color:#d0d0d0;"> عدد المنتجات  &nbsp;&nbsp;&nbsp;{{$productQTY}}<span></h3>
 
  <div class="row" id="load_more_item_section">
            <?php $getProduct_id=0;?>
	   @foreach($moreProducts as $product)
	     <div class="column5" product_id="{{$product->id}}" data-toggle="" data-target="#productModalSingle">
			   ...
			   <?php 
			       ...
			       // $allqty=DB::table('product_store')->where('product_id',$product->id)->get();
					...
					//	 $allQuantity=$allQuantity+$totalCount->qty; 
					 //}
			   ?>
			   <button class="btn add_to_cart" product_id="{{$product->id}}"  product_qty="{{$product->$product}}">اطلب الان</button> 			
			   <p style="display:none" class="english_name">{{$product->name}}</p>
			   <p style="" class="arabic_name">{{$product->arabic_name}}</p>
			</div>
		  </div> 
	   ...
	   @endforeach
               <div id="remove_div" class="col-12" style="margin-top:20px; text-align:center;">
		           <button name="btn_more" id="btn_more" data-pro="{{$getProduct_id}}" class="load_more_item_btn">مشاهدة المزيد</button>
	            </div>
	   </div>
 </div>	 
</div>
