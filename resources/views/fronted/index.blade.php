@extends('fronted-layout.main')
 ...
 <?php $discount="<script> document.write( parseInt(localStorage.getItem('userCouponValue')));</script>";?>
   <div class="fronted-content">
	...
	<ul class="category-list" style='text-align:center;'>
	  <li category_id="all" class="category_item category_active">كل المنتجات</li>
	    ...
	    <?php 
		  $product_qty=DB::table('products')->where('category_id',$categories->id)->get();
		   $all=0;
		   foreach($product_qty as $pr){
			  
			  $qty=DB::table('product_store')->where('product_id',$pr->id)->where('store_id',$user_store)->get();
                      if(count($qty)>0):
		                $all+=(int)$qty[0]->qty;
                      endif;
		...
		?>
		@if($all>0  && Auth::user()->role_id ==6)
	    <li category_id="{{$categories->id}}" class="category_item">{{$categories->name}} </li>
	    @endif
	...
	<div class="form-group mobile-cartegory-list">
	 <select class="form-control mobile-cartegory">
		...
		?>    
             @if($all>0 and  Auth::user()->role_id ==6)
	...
	<div class="fonted_products">
	 <div class="row">
	  <h3 style="margin-bottom:10px;" class="col-md-9"> كل المنتجات  <span style="float:left; font-size:20px; color:#d0d0d0;"> عدد المنتجات  &nbsp;&nbsp;&nbsp;{{$productQTY}}<span></h3>
	      ...
	      <div class="single_product">
		    <div class="product-image">
			   ...
			   <img src="@if($product->external_link==1) {{$product->image}} @else {{asset('public/images/product')}}/{{$product->image}} @endif" class="img-responsive">
			     ...
			     <h3 class="product-price" category="{{$product->category_id}}" real_price="{{$product->price}}" style="display:none" >@if(strpos($product->price,'.')===false) {{number_format($product->price,0,'.',',')}} @else{{rtrim(number_format($product->price,2,'.',','),".0")}} @endif $</h3>
   ...
   <div class="confrimMessage" style="display:none">
     <div class="icon">
	    <span class="fa fa-check"></span >
	 </div>
	 <div class="text">
	    <h2 style="color:#888383">تم بنجاح  </h2> 
        <p style="color:#888383"> تمت اضافة المنتج الى السلة بنجاح  </p>
	 </div>
	 <span class="closeConfirmMessage fa fa-close" style="float:left;margin-top:18px;margin-left:9px; color: #888383;"></span>
   </div>
        ...
        <button type="button" class="close" data-dismiss="modal">&times; <span style="font-size:14px">اغلاق</span></button>
         ...
         <div class="details_content">
          <div class="modal-body">
                   <p style="text-align:center;"><img style="width:45px;"src="{{asset('public/images/icons/loader.gif')}}"></p>
      ...
      <div class="modal-body" style="background:#fff;padding:0px; ">
          <div style='padding:16px 40px;'>
		     ...
		     <div style="text-align:center">
		        <img src="{{asset('public/images/icons/added-to-cart2.gif')}}">
		     </div>  
...
<div id="product_not_available" class="modal fade " role="dialog" >
  <div class="modal-dialog " style="width:325px; padding-top:80px ">
    <div class="modal-content" style="border-radius:0px;padding:20px;">
      <div class="modal-body" style="background:#fff;padding:0px; ">
        ...
        <div style="text-align:center;">
		   <h1 style="margin:20px 0px;">لا يوجد قطع كافية من هذا المنتج </h1>
		    <h2>يرجى التواصل مع الموزع لتعبئة المخزن</h2>
        </div>		
	 </div>
    </div>
  </div>
      ...
      <div class="modal-body">
	    <div class="row">
				...
				<div class="row">
				  <div class="sperator"></div>
				   <div class="col-sm-4">
						...
						<img src="{{asset('public/images/icons/customer.svg')}}">
				   </div>
				   <div class="col-sm-4">
				        <span class="circle noselect">
						    2
						</span>
						 <img src="{{asset('public/images/icons/voucher.svg')}}" style="margin-top:30px;">
				   </div>
				   <div class="col-sm-4">
				        <span class="circle noselect">
						    3
						</span>
						<img src="{{asset('public/images/icons/tax.svg')}}">
				   </div>
				</div>
			   </div>
			...
			<div class="col-sm-4 modal_image">
			  <img src="{{asset('public/images/icons/modal_image.jpg')}}">
			</div>
                  </div>
                <div class="row">
	      <div class="col-12">
...
<?php $zero_qty=DB::table('general_settings')->where('id','1')->get()[0]->zero_balance; ?>
 ...
 .confrimMessage{
    ...
    min-height: 73px;
    background: #fff;
    border-right: 4px solid #00e000;
    bottom:15px;
    right: 36%;
    box-shadow: 1px 3px 12px #bfb8b8;
    padding:12px;
	z-index:4000;
 }
 .confrimMessage .icon{
 ...
 $(document).ready(function () {
   ...
   $(document).on("click","#product_video .close", function(){
	...
	$(document).on("click",".add_to_cart",function(){
       ...
       var qty=$(this).attr('product_qty');
	   if(qty<=-negative_sale  && user_role !=8 /* && zero_qty==0 */){
		    $("#product_not_available").modal('show');
	   } 
      else
	  {
		  var qty=$(this).attr('product_qty',qty-1);    
         ...
         $.ajax({
			 ...
			 success:function(response){
                ...
                $(".confrimMessage").fadeIn();
				setTimeout(function() {$('.confrimMessage').fadeOut();}, 2000);
    ...
    $(document).on("change",".select_subCategory",function(){
       $('.css_preloader').css('display','block');
                $('.fonted_products').css('display','none');
