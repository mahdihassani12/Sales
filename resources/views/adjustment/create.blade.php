                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6"  style="display: none;">
                                        <div class="form-group">
                                            ...
                                            <p><strong>{{'adr-' . date("Ymd") . '-'. date("his")}}</strong></p>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label><strong>{{trans('file.Store')}} *</strong></label>
                                            <select @if($user_role!=2 and $user_role!=1) @endif required id="store_id" name="store_id" class="selectpicker form-control" data-live-search="true" data-live-search-style="begins" title="Select store...">
                                                @foreach($ezpos_store_list as $store)
                                                <option value="{{$store->id}}" @if($default_store==$store->id) {{'selected'}} @endif >{{$store->name}}</option>
                                                @endforeach
                                            </select>
											<input type="hidden" name="adjustment_type" value="in_store" >

                                            <input type="hidden" id="user_default_store" value="@if(isset($store_id)) {{$store_id}} @else {{$default_store}} @endif" role_id="{{$user_role}}">
                                           
                                           @if($user_role!=2 and $user_role!=1 and $user_role!=9 and $user_role!=11)
                                            <input type="hidden" name="store_id" value="{{$default_store}}">
                                           @endif
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <div class="form-group">
                                            <a href="{{asset('public/sample_file/sample_adjustment_insert.xls')}}" class="btn btn-info btn-block btn-md"><i class="fa fa-download"></i>  {{trans('file.Download')}} {{trans('file.Sample File')}}
                                            </a>
                                        </div>
                                        </div>
                                    </div>
	        ...
	        $.ajax({
	            ...
	            success: function(data) {
	                ...
	                if(flag){
	                    ...
	                    cols += '<td class="action"><select name="action[]" class="form-control act-val" style="display:none"><option value="-">{{trans("file.Subtraction")}}</option><option value="+" selected>{{trans("file.Addition")}}</option></select></td>';
